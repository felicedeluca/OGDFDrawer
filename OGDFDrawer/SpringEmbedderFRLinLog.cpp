 ...
 *        algorithm with LinLog force computations.
 *
...
SpringEmbedderFRLinLog::ArrayGraph::ArrayGraph(GraphAttributes &ga) : m_ga(&ga), m_mapNode(ga.constGraph())
{
...
SpringEmbedderFRLinLog::ArrayGraph::~ArrayGraph()
{
...
void SpringEmbedderFRLinLog::ArrayGraph::initCC(int i)
{
	System::alignedMemoryFree(m_orig);
	System::alignedMemoryFree(m_src);
	System::alignedMemoryFree(m_tgt);
...
SpringEmbedderFRLinLog::SpringEmbedderFRLinLog()
{
	// default parameters
	m_iterations = 1000;
	m_noise      = true;
...
void SpringEmbedderFRLinLog::call(GraphAttributes &AG)
{
...
void SpringEmbedderFRLinLog::initialize(ArrayGraph &component)
{
...
void SpringEmbedderFRLinLog::cool(double &tx, double &ty, int &cF)
{
...
void SpringEmbedderFRLinLog::mainStep(ArrayGraph &C)
{
	...
	while (!converged)
	{
		...
		double constant_factor_r = 1;
		double constant_factor_a = 20;
		...
		for(int v = 0; v < n; ++v)
		{
			...
			for(int u = 0; u < n; ++u)
			{
				...
				*/
				...
				double distSquare = max(minDistSquare, delta_x*delta_x + delta_y*delta_y);
				double t = 1 / distSquare;
				double fac_x = delta_x * t;
				double fac_y = delta_y * t;

				int deg_u = C.original(u)->degree();
				int deg_v = C.original(v)->degree();
				
				disp_x[v] += constant_factor_r * (deg_u*deg_v) * fac_x;
				disp_y[v] += constant_factor_r * (deg_u*deg_v) * fac_y;
		...
		for(int e = 0; e < C.numberOfEdges(); ++e)
		{
			...
			disp_x[v] -= constant_factor_a * delta_x / dist;
			disp_y[v] -= constant_factor_a * delta_y / dist;

			disp_x[u] += constant_factor_a * delta_x / dist;
			disp_y[u] += constant_factor_a * delta_y / dist;
...
void SpringEmbedderFRLinLog::mainStep_sse3(ArrayGraph &C)
{
